...
const allChatQNA = [{
    ...
    answer: 'Well honestly, this is one such question to which the  answer keeps changing over time. But right now, my goal is to keep you interested',
},{
    id: 2,
    ...
    answer: 'They do, but within yourself. It\'s called the subconsious. Pray to it to see the results',
},{
    id: 5,
    question: 'Best moment of your life so far?',
    answer: 'Every moment so far has been the best moment for me. Sorry for the awkward answer, but to be honest, whenever something great happens, we claim it to be the best moment, not because it is the best moment, but because we\'ve forgotten other great moments that have happened',
},{
    id: 6,
    question: 'Best memory of your travel or trip to some place',
    answer: 'I once went to a waterfall in Jalagampari, I touched paradise momentarily when the water fell on my head.',
},];

const headerText = 'Want to know more about me ? Let\'s chat then (I\'m not a spammer 😉)';
const subtitleText = 'Click on any of the questions in brown to start chatting.'

function About() {

  const [remainingQuestionsIds, setRemainingQuestionsIds] = useState([0,1,2,3,4,5,6]);
  const [answeredQuestionsIds, setAnsweredQuestionsIds] = useState([]);
  const [answeringAQuestion, setAnsweringAQuestion] = useState(false);
  const [questionIdCurrentlyAnswered, setQuestionIdCurrentlyAnswered] = useState(-1);
...
//   })
  ...
  const answerTheQuestion = (id: number) => {
    ...
    setRemainingQuestionsIds(remainingQuestionsIds.filter((remainingQuestionsId) => remainingQuestionsId != id ));
    setTimeout(() => {
  ...
  useEffect(() => {
    const remQuestionIds = allChatQNA.map((item) => item.id).filter((id) => !answeredQuestionsIds.includes(id));
    ...
    <div className={'sm:w-3/4 lg:w-3/5 xl:w-1/2 sm:ml-16 lg:ml-60 xl:ml-96 shadow-lg shadow-blue1 drop-shadow-lg min-h-3/4 h-60vh max-h-3/4 rounded-b-10xl inline-block  overflow-y-scroll'}>
        ...
        {answeredQuestionsIds.map((answeredQuestionsId) => {
                ...
                <div className={`flex justify-end`}>
                        <span className={`bg-blue1 text-right font-merriweather font-bold text-white m-2 p-6 rounded-t-5xl rounded-bl-5xl text-md `}
                        >
                    ...
                    <div className={`flex justify-start`}>
                        <span className={`max-w-xl bg-blue2 text-left font-merriweather font-bold text-blue1 m-2 p-6 rounded-t-5xl rounded-br-5xl text-md `}
                        >
            ...
            {/* Remaining Questions */}
                {!answeringAQuestion && remainingQuestionsIds.map((remainingQuestionsId) => {
                    return <div className={`flex justify-end`}>
                        <span className={`bg-brown2 text-right font-merriweather font-bold text-white m-2 p-3 rounded-t-5xl rounded-bl-5xl text-sm`}
                        >
                            <button onClick={() => {
